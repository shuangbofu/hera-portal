...
const { ADMIN, ANTD, CUSTOM } = require('../config/default')
...
function getThemeToggleColors(color, mode) {
  ...
  const customColors = CUSTOM[mode]
    .map(color => isHex(color) ? color : toNum3(color).join(','))
  // 内容色去重
  ...
  return { primary, mainColors, subColors, menuColors, contentColors, rgbColors, functionalColors, customColors }
}

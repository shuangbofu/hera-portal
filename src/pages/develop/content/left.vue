  ...
  computed: {
        ...
        const filterData = nodesAfterFilter(
          ...
          (i, value) => {
            ...
            if (value.includes(" ")) {
              ...
              if (valueArr) {
                return (
                  valueArr.map(value => f(value)).filter(i => i).length > 0
                );
              }
            }
            // 名称/单个ID
    ...
    treeCache() {
      return function(tab) {
        ...
        // 过滤之后所有满足条件展开的(父)结点
        const expandedKeys = this.treeDataTuple(tab).expandedNodes.map(
          i => i.key
        );
        // 过滤之后关闭过的结点
        const excludeExpandedKeys = this.filterObj.excludeNodes;
        const filterExpanedKeys = expandedKeys.filter(
          i =>
            excludeExpandedKeys.length === 0 || !excludeExpandedKeys.includes(i)
        );
        // 查询后展开的结点
        return expandedKeys.length > 0
          ? { ...treeCache, expandedKeys: filterExpanedKeys }
          : treeCache;
      };
    }
  },
